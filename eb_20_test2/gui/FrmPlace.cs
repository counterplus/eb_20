...
namespace eb_20_test2.gui
{
    public partial class FrmPlace : Form
    {
        ...
        public FrmPlace(EB20Control ebc)
        {
            InitializeComponent();
            ebC = ebc;
            initConfig();
        ...
        {
            plc = new Place();
            plc.place_id = txtId.Text;
            plc.place_name = txtName.Text;
            plc.active = "1";
        }
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("ต้องการ บันทึกข้อมูล", "Save Data", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.OK)
            {
                setStaff();
                int chk = 0;
                String re = ebC.eB20DB.plcDB.insertPlace(plc, "");
        ...
        private void setGrd()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    grv1[colId, i].Value = dt.Rows[i][ebC.eB20DB.plcDB.plc.place_id].ToString();
                    grv1[colName, i].Value = dt.Rows[i][ebC.eB20DB.plcDB.plc.place_name].ToString();
