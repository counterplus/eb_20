...
{
    public partial class FrmLoca : Form
    {
        EB20Control ebC;
        int colId = 0, colName = 1;
        Location loca;

        public FrmLoca(EB20Control ebc)
        {
            InitializeComponent();
            ebC = ebc;
            initConfig();
        ...
        {
            loca = new Location();
            loca.loca_id = txtId.Text;
            loca.loca_name = txtName.Text;
            loca.active = "1";
        }
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("ต้องการ บันทึกข้อมูล", "Save Data", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.OK)
            {
                setStaff();
                int chk = 0;
                String re = ebC.eB20DB.lctDB.insertLocation(loca, "");
        ...
        private void setGrd()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    grv1[colId, i].Value = dt.Rows[i][ebC.eB20DB.lctDB.lct.loca_id].ToString();
                    grv1[colName, i].Value = dt.Rows[i][ebC.eB20DB.lctDB.lct.loca_name].ToString();
        ...
        private void FrmLoca_Load(object sender, EventArgs e)
        {
